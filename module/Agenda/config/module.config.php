<?php
/**
 * Zend Framework (http://framework.zend.com/)
 *
 * @link      http://github.com/zendframework/ZendSkeletonApplication for the canonical source repository
 * @copyright Copyright (c) 2005-2015 Zend Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd New BSD License
 */

namespace Agenda;

return array(
    'router' => array(
        'routes' => array(
            'home_agenda' => array(
                'type' => 'Zend\Mvc\Router\Http\Literal',
                'options' => array(
                    'route'    => '/agenda',
                    'defaults' => array(
                        'controller' => 'Agenda\Controller\Index',
                        'action'     => 'index',
                    ),
                ),
            ),
            // The following is a route to simplify getting started creating
            // new controllers and actions without needing to create a new
            // module. Simply drop new controllers in, and you can access them
            // using the path /application/:controller/:action
            'agenda' => array(
                'type'    => 'Literal',
                'options' => array(
                    'route'    => '/agenda/index',
                    'defaults' => array(
                        '__NAMESPACE__' => 'Agenda\Controller',
                        'controller'    => 'Index',
                        'action'        => 'index',
                    ),
                ),
                'may_terminate' => true,
                'child_routes' => array(
                    'default' => array(
                        'type'    => 'Segment',
                        'options' => array(
                            'route'    => '/agenda/[:controller[/:action]]',
                            'constraints' => array(
                                'controller' => '[a-zA-Z][a-zA-Z0-9_-]*',
                                'action'     => '[a-zA-Z][a-zA-Z0-9_-]*',
                            ),
                            'defaults' => array(
                            ),
                        ),
                    ),
                ),
            ),

            'grupo_index' => array(
                'type' => 'Segment',
                'options' => array(
                    'route'    => '/agenda/grupo[/]',
                    'defaults' => array(
                        'controller' => 'Agenda\Controller\Grupo',
                        'action'     => 'index',
                    ),
                ),
            ),
            'grupo_adicionar' => array(
                'type' => 'Segment',
                'options' => array(
                    'route'    => '/agenda/grupo/adicionar[/]',
                    'defaults' => array(
                        'controller' => 'Agenda\Controller\Grupo',
                        'action'     => 'adicionar',
                    ),
                ),
            ),
            'grupo_editar' => array(
                'type' => 'Segment',
                'options' => array(
                    'route'    => '/agenda/grupo/editar/[:id_grupo][/]',
                    'defaults' => array(
                        'controller' => 'Agenda\Controller\Grupo',
                        'action'     => 'adicionar',
                    ),
                ),
            ),
            'grupo_listar' => array(
                'type' => 'Segment',
                'options' => array(
                    'route'    => '/agenda/grupo/listar[/]',
                    'defaults' => array(
                        'controller' => 'Agenda\Controller\Grupo',
                        'action'     => 'listar',
                    ),
                ),
            ),
            'grupo_remover' => array(
                'type' => 'Segment',
                'options' => array(
                    'route'    => '/agenda/grupo/remover/[:id_grupo][/]',
                    'defaults' => array(
                        'controller' => 'Agenda\Controller\Grupo',
                        'action'     => 'remover',
                    ),
                ),
            ),
        ),
    ),
    'service_manager' => array(
        'abstract_factories' => array(
            'Zend\Cache\Service\StorageCacheAbstractServiceFactory',
            'Zend\Log\LoggerAbstractServiceFactory',
        ),
        'factories' => array(
            'translator' => 'Zend\Mvc\Service\TranslatorServiceFactory',
        ),
    ),
    'translator' => array(
        'locale' => 'en_US',
        'translation_file_patterns' => array(
            array(
                'type'     => 'gettext',
                'base_dir' => __DIR__ . '/../language',
                'pattern'  => '%s.mo',
            ),
        ),
    ),
    'controllers' => array(
        'invokables' => array(
            'Agenda\Controller\Index' => Controller\IndexController::class,
            'Agenda\Controller\Grupo' => Controller\GrupoController::class
        ),
    ),
    'view_manager' => array(
        'display_not_found_reason' => true,
        'display_exceptions'       => true,
        'doctype'                  => 'HTML5',
        'not_found_template'       => 'error/404',
        'exception_template'       => 'error/index',
        'template_map' => array(
            //'layout/layout'           => __DIR__ . '/../view/layout/layout.phtml',
            'layout/layout'           => __DIR__ . '/../../Application/view/layout/layout.phtml',
            'agenda/index/index' => __DIR__ . '/../view/agenda/index/index.phtml',
            'error/404'               => __DIR__ . '/../view/error/404.phtml',
            'error/index'             => __DIR__ . '/../view/error/index.phtml',
        ),
        'template_path_stack' => array(
            __DIR__ . '/../view',
        ),
    ),
    // Placeholder for console routes
    'console' => array(
        'router' => array(
            'routes' => array(
            ),
        ),
    ),
);
